...
/* 3 */
SELECT TollGate.TGLength, TollGate.ID
FROM TollGate
...
SELECT License
FROM Vehicle
WHERE Vehicle.ETag in
(SELECT ETag
FROM ETCReading
WHERE ETCReading.TGID in
(SELECT ID
FROM TollGate
WHERE TollGate.TWID = 1)
...
FROM
	(
	(SELECT Driver.FullName, Driver.License
	FROM Driver) AS D
	...
	(SELECT ETCReading.ID, Vehicle.License, Vehicle.TollRate * COUNT
		 (
			(SELECT ETCReading.ETAG
		FROM ETCReading
		WHERE ETCReading.ETAG = Vehicle.ETag) 
		 ) as spent
...
SELECT Driver.FullName,
	COUNT( * ) AS Readings
FROM Driver, ETCReading, Vehicle
...
SELECT D.FullName, ETC.Readings
FROM
	(
	SELECT Vehicle.License, COUNT (ETCReading.ID) as Readings
	FROM Vehicle, ETCReading
...
/* 3 */
/* Get the gates that aren't well used */
SELECT TollGate.ID
FROM TollGate
WHERE TollGate.ID in 
(SELECT TGID
